...
namespace Auth.GG_Winform_Example
{
    ...
    internal class OnProgramStart
    {
        ...
        public static void Initialize(string name, string aid, string secret, string version)
        {
            ...
            {
                //.Show("Failed to initialize your application correctly in Program.cs!", Name, //Button.OK, //Image.Error);
            ...
            using (WebClient wc = new WebClient())
            {
                ...
                try
                {
                    ...
                    {
                        //.Show("Possible malicious activity detected!", OnProgramStart.Name, //Button.OK, //Image.Warning);
                        Process.GetCurrentProcess().Kill();
                    }
                    ...
                    {
                        //.Show("Security error has been triggered!", Name, //Button.OK, //Image.Error);
                        Process.GetCurrentProcess().Kill();
                    }
                    switch (response[2])
                    {
                        case "success":
                            ...
                            {
                                //.Show("Application is in Developer Mode, bypassing integrity and update check!", Name, //Button.OK, //Image.Warning);
                                ...
                                //.Show("Your applications hash has been saved to integrity.txt, please refer to this when your application is ready for release!", Name, //Button.OK, //Image.Information);
                            }
                            else
                            {
                                if (response[12] == "Enabled")
                                {
                                    ...
                                    {
                                        //.Show($"File has been tampered with, couldn't verify integrity!", Name, //Button.OK, //Image.Error);
                                        Process.GetCurrentProcess().Kill();
                                    }
                                }
                                if (ApplicationSettings.Version != Version)
                                {
                                    //.Show($"Update {ApplicationSettings.Version} available, redirecting to update!", Name, //Button.OK, //Image.Error);
                            ...
                            {
                                //.Show("Looks like this application is disabled, please try again later!", Name, //Button.OK, //Image.Error);
                                Process.GetCurrentProcess().Kill();
                            }
                            break;
                        case "binderror":
                            //.Show(Encryption.Decode("RmFpbGVkIHRvIGJpbmQgdG8gc2VydmVyLCBjaGVjayB5b3VyIEFJRCAmIFNlY3JldCBpbiB5b3VyIGNvZGUh"), Name, //Button.OK, //Image.Error);
                            Process.GetCurrentProcess().Kill();
                            return;
                        case "banned":
                            //.Show("This application has been banned for violating the TOS" + Environment.NewLine + "Contact us at support@auth.gg", Name, //Button.OK, //Image.Error);
                            Process.GetCurrentProcess().Kill();
                            return;
                    }
                    Security.End();
                ...
                {
                    //.Show(ex.Message, Name, //Button.OK, //Image.Error);
                    Process.GetCurrentProcess().Kill();
                }
            }
        }
    ...
    internal class API
    {
        public static void Log(string username, string action)
        {
            ...
            {
                //.Show("Please initialize your application first!", OnProgramStart.Name, //Button.OK, //Image.Error);
                Process.GetCurrentProcess().Kill();
            }
            if (string.IsNullOrWhiteSpace(action))
            {
                //.Show("Missing log information!", ApplicationSettings.Name, //Button.OK, //Image.Error);
                Process.GetCurrentProcess().Kill();
            }
            string[] response = new string[] { };
            using (WebClient wc = new WebClient())
            {
                ...
                {
                    //.Show(ex.Message, OnProgramStart.Name, //Button.OK, //Image.Error);
                    Process.GetCurrentProcess().Kill();
                }
            }
        }
        public static void UploadPic(string username, string path)
        {
            ...
            {
                //.Show("Invalid Picture information!", ApplicationSettings.Name, //Button.OK, //Image.Error);
                Process.GetCurrentProcess().Kill();
            }
            string[] response = new string[] { };
            using (WebClient wc = new WebClient())
            {
                ...
                try
                {
                    ...
                    {
                        case "success":
                            //.Show("Successfully updated profile picture!", OnProgramStart.Name, //Button.OK, //Image.Information);
                            Security.End();
                            return;
                        case "permissions":
                            //.Show("Please upgrade your plan to use this feature!", OnProgramStart.Name, //Button.OK, //Image.Error);
                            Security.End();
                            return;
                        case "maxsize":
                            //.Show("Image cannot be greater than 1 MB!", OnProgramStart.Name, //Button.OK, //Image.Error);
                            Security.End();
                            return;
                        case "failed":
                            //.Show("Failed to upload profile picture!", OnProgramStart.Name, //Button.OK, //Image.Error);
                            Security.End();
                            return;
                    }
                }
        ...
        public static bool AIOLogin(string AIO)
        {
            ...
            {
                //.Show("Missing user login information!", ApplicationSettings.Name, //Button.OK, //Image.Error);
                Process.GetCurrentProcess().Kill();
            }
            string[] response = new string[] { };
            using (WebClient wc = new WebClient())
            {
                try
                {
                    ...
                    if (response[0] != Constants.Token)
                    {
                        //.Show("Security error has been triggered!", OnProgramStart.Name, //Button.OK, //Image.Error);
                        Process.GetCurrentProcess().Kill();
                    }
                    if (Security.MaliciousCheck(response[1]))
                    {
                    ...
                    switch (response[2])
                    {
                        ...
                        case "invalid_details":
                            ...
                            return false;
                        case "time_expired":
                            //.Show("Your subscription has expired!", ApplicationSettings.Name, //Button.OK, //Image.Warning);
                            Security.End();
                            return false;
                        case "hwid_updated":
                            //.Show("New machine has been binded, re-open the application!", ApplicationSettings.Name, //Button.OK, //Image.Information);
                            Security.End();
                            return false;
                        case "invalid_hwid":
                            //.Show("This user is binded to another computer, please contact support!", ApplicationSettings.Name, //Button.OK, //Image.Error);
                            Security.End();
                            return false;
                    }
                }
                ...
                {
                    //.Show(ex.Message, ApplicationSettings.Name, //Button.OK, //Image.Error);
                    Security.End();
                    Process.GetCurrentProcess().Kill();
                }
                return false;
        ...
        public static bool AIORegister(string AIO)
        {
            ...
            if (string.IsNullOrWhiteSpace(AIO))
            {
                //.Show("Invalid registrar information!", ApplicationSettings.Name, //Button.OK, //Image.Error);
                Process.GetCurrentProcess().Kill();
            }
            string[] response = new string[] { };
            using (WebClient wc = new WebClient())
                ...
                try
                {
                    ...
                    switch (response[2])
                    {
                        case "success":
                            ...
                            return true;
                        case "invalid_details":
                            //.Show("Sorry, your username/password does not match!", ApplicationSettings.Name, //Button.OK, //Image.Error);
                            Security.End();
                            return false;
                        case "time_expired":
                            //.Show("Your subscription has expired!", ApplicationSettings.Name, //Button.OK, //Image.Warning);
                ...
                try
                {
                    ...
                    switch (response[2])
                    {
                        ...
                        case "invalid_license":
                            //.Show("License does not exist!", ApplicationSettings.Name, //Button.OK, //Image.Error);
                            Security.End();
                            return false;
                        case "email_used":
                            //.Show("Email has already been used!", ApplicationSettings.Name, //Button.OK, //Image.Error);
                            Security.End();
                            return false;
                        case "invalid_username":
                            //.Show("You entered an invalid/used username!", ApplicationSettings.Name, //Button.OK, //Image.Error);
                            Security.End();
                            return false;
                    }
                }
                ...
                try
                {
                    ...
                    switch (response[2])
                    {
                        case "success":
                            ...
                            return true;
                        case "invalid_token":
                            //.Show("Token does not exist!", ApplicationSettings.Name, //Button.OK, //Image.Error);
                            Security.End();
                            return false;
                        case "invalid_details":
                            //.Show("Your user details are invalid!", ApplicationSettings.Name, //Button.OK, //Image.Error);
                            Security.End();
                            return false;
                    }
                }
    ...
    internal class Security
    {
        ...
        public static void Start()
        {
            ...
            {
                //.Show("A session has already been started, please end the previous one!", OnProgramStart.Name, //Button.OK, //Image.Warning);
                Process.GetCurrentProcess().Kill();
            }
            else
            {
                using (StreamReader sr = new StreamReader($@"{drive}Windows\System32\drivers\etc\hosts"))
                {
                    ...
                    {
                        Constants.Breached = true;
                        //.Show("DNS redirecting has been detected!", OnProgramStart.Name, //Button.OK, //Image.Error);
                        Process.GetCurrentProcess().Kill();
                    }
                }
        ...
        public static void End()
        {
            if (!Constants.Started)
            {
                //.Show("No session has been started, closing for security reasons!", OnProgramStart.Name, //Button.OK, //Image.Warning);
                Process.GetCurrentProcess().Kill();
            }
            else
            {
    ...
    class InfoManager
    {
        ...
        private void OnCallBack()
        {
            ...
            {
                Constants.Breached = true;
                //.Show("ARP Cache poisoning has been detected!", OnProgramStart.Name, //Button.OK, //Image.Error);
                Process.GetCurrentProcess().Kill();
            }
            else
            {
